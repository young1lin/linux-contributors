...
基于 Claude Code Agent 的 AI 驱动 Linux 内核提交分析工具，带 GUI 可视化界面。
...
# 初始化 Linux 内核仓库（用于查看完整代码变更）
git submodule update --init --recursive

# 启动 GUI 可视化工具
uv run python gui_viewer.py
```

> ⚠️ **重要提示**：如需查看完整的代码变更（带 Git 风格语法高亮），必须先初始化 `linux-kernel` 子仓库。如果不初始化，只能查看 AI 提取的代码片段。

### GUI 主要功能

- **🏆 公司排名**：显示所有中国公司的贡献排名，支持中英文搜索
- **📊 统计图表**：平均分、总评分、分类分布、提交数量可视化
- **📋 提交详情**：分页加载（每页100条），支持华为等大数据量，**点击表头可排序**
- **🔍 代码查看**：Git 风格语法高亮（🔴红色删除、🟢绿色新增）
...
![公司排名和统计图表](排名.png)
*左侧：公司排名表格，支持搜索和排序。右侧：统计图表（平均分、总评分、分类分布饼图、提交数量）*

![提交详情表格](代码详情.png)
*提交详情表格，分页显示（每页100条），支持华为等大数据量查看。点击表头可按任意列排序*

![右键菜单](右键.png)
*右键点击提交行，可查看代码片段、详细分析、打开链接或复制哈希*

![表头排序](排序.png)
*点击表头即可排序（日期、作者、评分、分类等），再次点击切换升序/降序*
...
本项目包含一个功能完整的 PyQt6 桌面应用，用于可视化和分析 Linux 内核中国公司贡献数据。
...
- 支持中英文公司名搜索
- 显示最高分和最低分统计
- **公司名中文显示**：表格显示中文（华为、联发科等），鼠标悬停显示英文名
...
#### 🎨 代码查看
- **AI 摘要片段**：显示 AI 提取的代码片段
- **完整代码变更**：点击"显示完整变更"从本地仓库获取完整 diff
- **Git 风格语法高亮**：
  - 🔴 红色 = 删除的行
  - 🟢 绿色 = 新增的行
  - 🔵 蓝色 = diff 头部信息
...
GUI 会自动加载 `data/` 目录下的数据文件：
- `chinese_companies_*.jsonl` - 提交记录数据
- `chinese_companies_*_summary.json` - 公司汇总数据

### 中文显示

GUI 完全支持中文显示：
- 所有界面文本使用中文
- 分类代码自动翻译（BUG-CRASH → 崩溃修复）
- 评分维度翻译（technical → 技术难度）
- 公司名中文显示（华为、联发科等）
- PyQt6 使用 UTF-8 编码，避免 Windows GBK 乱码问题
